...
const DoctorWaitingList = () => {
  ...
  useEffect(() => {
    const decodeToken = () => {
      ...
      if (!token) {
        ...
        if (decoded.userId.substring(0, 2) === "BS") setValidAccess(true);
  ...
  const handleMedicalReportModal = (waitId) => {
    setSelectedPatient(
      waitingList.flat().find((patient) => patient.wait_id === waitId) || {}
    );
  ...
  useEffect(() => {
    ...
    if (waitingList.length > 0) {
      ...
      const firstWaitId = waitingList[0][0]?.wait_id || null;
      setSelectedPatient(
        waitingList.flat().find((patient) => patient.wait_id === firstWaitId) ||
          {}
      );
    }
  }, [waitingList]);
      ...
      <div>
        ...
        >
          ...
          >
            ...
            <Modal
              show={showMedicalReportModal}
              onHide={handleMedicalReportModal}
                    ...
                    <Dropdown
                      onSelect={(eventKey) => {
                        const selectedService = services.find(
                          (service) =>
                            service.service_id === parseInt(eventKey, 10)
                        );
                        setSelectedServices([
                          ...selectedServices,
                          { ...selectedService, quantity: serviceQuantity },
                        ]);
                        setServiceQuantity("");
                      }}
                    >
                  ...
                  {selectedServices.length > 0 && (
                    <>
                      {selectedServices.map((selectedService, index) => (
                        <div key={index}>
                          <strong  style={{verticalAlign:"middle", marginTop: "8px"}}>
                    ...
                    <>
                      {selectedDrugs.map((selectedDrug, index) => (
                        <div key={index}>
                          ...
                          <strong style={{verticalAlign:"middle", marginTop: "8px"}}>{selectedDrug.drug_name}</strong>
                          <Form.Group controlId={`quantity-${index}`} style={{ width: "35%", marginLeft: "10px" }}>
                            {/* <Form.Label>Quantity</Form.Label> */}
                            <Form.Control
                              type="text"
                    ...
                    <Dropdown
                      onSelect={(eventKey) => {
                        const selectedEquipment = equipments.find(
                          (equipment) =>
                            equipment.equipment_id === parseInt(eventKey, 10)
                        );
                      ...
                      <Dropdown.Menu>
                        {equipments.map((equipment) => (
                          <Dropdown.Item
                            ...
                            eventKey={equipment.equipment_id}
                          >
                            {equipment.name}
                          </Dropdown.Item>
                        ))}
                    ...
                    <>
                      {selectedEquipments.map((selectedEquipment, index) => (
                        <div key={index}>
                          ...
                          <strong style={{verticalAlign:"middle", marginTop: "8px"}}>{selectedEquipment.name}</strong>
                          <div style = {{width: "50%"}}>
                          <Form.Group controlId={`equipment-quantity-${index}`} style={{ width: "100%", marginLeft: "10px" }}>
                            {/* <Form.Label>Quantity</Form.Label> */}
                            <Form.Control
                              type="text"
                              placeholder="Quantity"
                              ...
                              onChange={(e) => {
                                ...
                                updatedEquipments[index].quantity =
                                  e.target.value;
                          ...
                          <Form.Group controlId={`equipment-days-${index}`} style={{ width: "100%", marginLeft: "10px" }}>
                            ...
                            <Form.Control
                              ...
                              onChange={(e) => {
                                ...
                                updatedEquipments[index].daysUsed =
                                  e.target.value;
                                setSelectedEquipments(updatedEquipments);
                              }}
                            />
